...
func init() {
	...
	devCmdUp.Flags().StringVar(&params2.SourceRef, "ref", "", "use for overriding source ref or branch ref in buildconfig.")
	devCmdUp.Flags().StringSliceVar(&CVars, "cvars", []string{}, "Slice of key=value pairs, seperated by ,")
	devCmdUp.Flags().StringVar(&params2.PropertiesFile, "cvfile", "", "Property file with component configuration values. Can be generated with \"mg generate properties\" command.)")
	...
	devCmdUp.Flags().StringVarP(&params2.OutputImagestream, "istream", "i", "", "specify if you want to output to another imagestream than the component name")
	...
	devCmdUp.Flags().BoolVar(&params2.ServiceMonitor, "service-monitor", false, "Set flag to also create a ServiceMonitor resource. Requires a cluster with the prometheus-operator.")
	devCmdUp.Flags().Int32Var(&params2.ServiceMonitorPort, "service-monitor-port", params2.ServiceMonitorPort, "Set Service port to scrape in ServiceMonitor.")
	devCmdUp.Flags().StringVar(&params2.ServiceMonitorOperatorName, "service-monitor-operator-name", params2.ServiceMonitorOperatorName, "Name of prometheus-operator instance to create ServiceMonitor for.")
	...
	devCmdDown.Flags().BoolVar(&params2.Everything, "everything", false, "Delete all resources and artifacts generated from mg dev up.")
