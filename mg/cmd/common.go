...
// Returns a metagraf.MGProperties struct of addressable fields
// in a metaGraf specification from a properties file generated by
// "mg generate properties" and provided through
// the --cvfile argument. Reading from files we assume that the
// metagraf.Property is required.
func PropertiesFromFile() metagraf.MGProperties {
	vars := metagraf.MGProperties{}
...
func MergeSourceKeyedVars(base metagraf.MGProperties, override map[string]string) metagraf.MGProperties {
	log.V(1).Info("MergeSourcevars(): base mg vars", base)
	log.V(1).Info("MergeSourceVars(): with override values: ", override)

	for _,prop := range base {
		if _, ok := override[prop.Key] ; ok {
			prop.Value = override[prop.Key]
		}
	}
